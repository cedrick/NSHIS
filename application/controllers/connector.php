...
class Connector extends CI_Controller {
	...
	function add()
	{
		...
		else
		{
			...
			if($id)
			{
				$this->devicelog->insert_log($this->session->userdata('user_id'), $id, 'connector', 'add');
	...
	function assign($location)
	{
		...
		else
		{
			...
			if ($id)
			{
				$this->devicelog->insert_log($this->session->userdata('user_id'), $connector_id, 'connector', 'assign', $id);
	...
	function edit($connector_id = NULL)
	{
		if (isset($connector_id))
		{
			...
			else
			{
				...
				if ($id)
				{
					$this->devicelog->insert_log($this->session->userdata('user_id'), $id, 'connector', 'edit');
	...
	function transfer($connector_id = NULL)
	{
		if (isset($connector_id))
		{
			...
			else
			{
				...
				if ($id)
				{
					$this->devicelog->insert_log($this->session->userdata('user_id'), $connector_id, 'connector', 'transfer', $id);
	...
	function comment($connector_id)
	{
		...
		else
		{
			...
			if ($id)
			{
				$this->devicelog->insert_log($this->session->userdata('user_id'), $id, 'connector', 'comment');
