...
class CPU extends CI_Controller {
	...
	function add()
	{
		...
		else
		{
			...
			if($id)
			{
				$this->devicelog->insert_log($this->session->userdata('user_id'), $id, 'cpu', 'add');
	...
	function assign($location)
	{
		...
		else
		{
			...
			if ($id)
			{
				$this->devicelog->insert_log($this->session->userdata('user_id'), $cpu_id, 'cpu', 'assign', $id);
	...
	function edit($cpu_id = NULL)
	{
		if (isset($cpu_id))
		{
			...
			else
			{
				...
				if($id)
				{
					$this->devicelog->insert_log($this->session->userdata('user_id'), $id, 'cpu', 'edit');
	...
	function delete($cpu_id)
	{
		...
		else
		{
			...
			else
			{
				...
				if ($id)
				{
					$this->devicelog->insert_log($this->session->userdata('user_id'), $id, 'cpu', 'delete');
	...
	*/
	...
	function comment($cpu_id)
	{
		...
		else
		{
			...
			if ($id)
			{
				$this->devicelog->insert_log($this->session->userdata('user_id'), $id, 'cpu', 'comment');
